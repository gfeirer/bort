...
Spec::Runner.configure do |config|
  ...
  def login_as_no_role
    ...
    @current_user.stub!(:has_role?).with(any_args()).and_return(false)
  end
  ...
  def login_as_scaffold_role
    @current_user = mock_model(User, :id => 4, :state => 'active', :roles => [Role.create(:name => "scaffold_role")])
    controller.stub!(:current_user).and_return(@current_user)
    @current_user.should_receive(:has_role?) do |role|
      if role == "scaffold_role"
        true
      else
        false
      end

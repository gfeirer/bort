...
describe SessionsController do
      ...
      [ true, false ].each do |want_remember_me|
        describe "my request cookie token is #{has_request_token.to_s}," do
          describe "and ask #{want_remember_me ? 'to' : 'not to'} be remembered" do
            ...
            it "greets me nicely"            do do_create; response.flash[:notice].should =~ /Sesión iniciada correctamente/   end
  ...
  describe "on failed login" do
    ...
    it 'flashes an error'           do do_create; flash[:error].should =~ /Nombre de usuario o contraseña incorrectos/ end

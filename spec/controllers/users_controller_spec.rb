...
describe UsersController do
  ...
  it 'does not allow creating users if not logged in as admin' do
    lambda do
  ...
  it 'allows creating users if logged in as admin' do
    lambda do
      login_as_admin
      create_user
      assigns(:user).reload
  ...
  it 'creates users in pending state' do
    login_as_admin
    create_user
    assigns(:user).reload
  ...
  #suspend account
  it 'suspends user if logged in as admin' do
    login_as_admin
    user = User.find_by_login('quentin')
    get :suspend, :id => user.id
    user.reload
    user.state.should == 'suspended'
    response.should redirect_back_or_default(root_path)
    flash[:notice].should == "Usuario desactivado con éxito."
    flash[:error ].should be_nil
  end

  it 'does not suspend user if not logged in as admin' do
    login_as_no_role
    user = User.find_by_login('old_password_holder')
    get :suspend, :id => user.id
    flash[:notice].should be_nil
    flash[:error ].should eql("No está autorizado para ver este contenido.")
    user.state.should == 'active'
  end

  it 'does not suspend user if already suspended' do
    login_as_admin
    user = User.find_by_login('quentin')
    user.suspend!
    user.state.should == 'suspended'
    ...
    response.should redirect_back_or_default(root_path)
    flash[:notice].should be_nil
    flash[:error ].should == "El usuario ya está desactivado."
  end

#  it 'does not suspend the user that is logged in, regardless of his role' do
#    login_as_admin
#    get :suspend, :id => @current_user.id
#    @current_user.state.should == 'active'
#    response.should redirect_back_or_default(root_path)
#    flash[:notice].should be_nil
#    flash[:error ].should == "No puedes desactivar tu propia cuenta"
#  end

  it 'does not suspend a non existing user' do
    login_as_admin
    get :suspend, :id => 999999
    flash[:notice].should be_nil
    flash[:error ].should == 'No se ha encontrado el usuario'
    response.should redirect_back_or_default(root_path)
  end
